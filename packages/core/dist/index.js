import { startOfToday, format, parse, isValid, getDaysInMonth, getDay, addDays, addMonths, addYears } from "date-fns";
...
class GregorianAdapter extends BaseCalendarAdapter {
   ...
   */
  parse(dateStr, formatStr, locale) {
    try {
      const jsDate = parse(dateStr, formatStr, /* @__PURE__ */ new Date(), {
  ...
  getWeekdayNames(locale) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      names.push(format(date, "EEEEEE", { locale: dateFnsLocale }));
    }
    return names;
  }
}
...
function generateCalendarMonth(year, month, adapter, locale, selectedDate, selectedDates) {
...
function getNavigatedDate(currentDate, direction, addDays2) {
  switch (direction) {
    case "left":
      return addDays2(currentDate, -1);
    case "right":
      return addDays2(currentDate, 1);
    case "up":
      return addDays2(currentDate, -7);
    case "down":
      return addDays2(currentDate, 7);
  }
}
